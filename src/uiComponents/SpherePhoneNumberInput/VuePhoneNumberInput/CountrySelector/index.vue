<template>
  <div ref="parent" :class="[{
    ...
    'is-valid': valid
  }, size]" class="country-selector" @blur.capture="handleBlur" @mouseenter="updateHoverState(true)"
    @mouseleave="updateHoverState(false)">
    <div v-if="value && !noFlags" class="country-selector__country-flag" @click.stop="toggleList">
      <div :class="`iti-flag-small iti-flag ${value.toLowerCase()}`" />
    </div>
    <input :id="id" ref="CountrySelector" :value="callingCode" :placeholder="label" :disabled="disabled"
      class="country-selector__input" readonly
      :style="[radiusLeftStyle, inputBorderStyle, inputBoxShadowStyle, inputBgColor]" @focus="isFocus = true"
      @click.stop="toggleList">
    <div class="country-selector__toggle" @click.stop="toggleList">
      <slot name="arrow">
        <svg mlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
    ...
    <label ref="label" :style="[labelColorStyle]" class="country-selector__label" @click.stop="toggleList">
      {{ hint || label }}
    </label>
    <Transition name="slide">
      <div v-show="hasListOpen" ref="countriesList" class="country-selector__list"
        :class="{ 'has-calling-code': showCodeOnList }" :style="[radiusStyle, listHeight, inputBgColor]">
        <v-text-field @keydown="keyboardNav" class="px-3 py-0 select-fixed-text" autofocus hide-details
          clearable></v-text-field>
        <RecycleScroller v-slot="{ item }" :items="countriesSorted" :item-size="1" key-field="iso2">

          <button :key="`item-${item.code}`" :class="[
            { 'selected': value === item.iso2 },
            { 'keyboard-selected': value !== item.iso2 && tmpValue === item.iso2 }
          ]" class="flex align-center country-selector__list__item" :style="[
  itemHeight,
  value === item.iso2 ? bgItemSelectedStyle : null
]" tabindex="-1" type="button" @click.stop="updateValue(item.iso2)">
            ...
            <span v-if="showCodeOnList" class="country-selector__list__item__calling-code flex-fixed">+{{ item.dialCode
            }}</span>
            <div class="dots-text">
              {{ item.name }}
            </div>
